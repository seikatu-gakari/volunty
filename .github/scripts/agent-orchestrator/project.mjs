...
export class ProjectStore {
  ...
  async transitionIssue(issueId, target, allowedFrom) {
    ...
    if (!Array.isArray(allowedFrom) || allowedFrom.some((status) => typeof status !== 'string')) {
      throw new Error('allowedFrom must be an array of status names');
    }

    const item = await this.ensureIssueItem(issueId);
    // PATCH直前に読むことで、他ワーカーの遷移を古い一覧結果で上書きしない。
    const current = await this.getIssueStatus(issueId);
    ...
    if (!allowedFrom.includes(/** @type {string} */ (current))) {
      throw new Error(`Current Status is not in allowedFrom: ${current ?? 'unset'}`);
    }

    await this.client.write(`${this.#basePath()}/items/${requireId(item.id, 'project item.id')}`, {
      method: 'PATCH',
