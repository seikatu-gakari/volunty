...
function currentPullRequest() { return { number: 30, head: { ref: 'cursor/issue-20-task', sha: 'abcdef' } }; }
...
function event(workflowRun, { relations = null, branch = 'cursor/issue-20-task', relationHeadRef = branch, relationHeadSha = workflowRun.headSha, headRepository = { id: 100, full_name: 'octo-org/widgets' } } = {}) {
  const pullRequests = relations ?? [{
    ...relation(),
    head: { ...relation().head, ref: relationHeadRef, sha: relationHeadSha },
  }];
  return {
    action: 'completed',
...
test('pull_request_targetのtop-level cursor/head SHAをPR headとしてrelation baseと比較せず処理する', async () => {
...
test('pull_requestsがemptyの実API shapeもtrusted head branchからexactly one PRを再解決する', async () => {
  const { repository, handlers } = setup();
  const failed = run(11, 'failure');
  repository.runs.push(failed);
  const branches = [];
  const findOpenPullRequestsByHead = repository.findOpenPullRequestsByHead.bind(repository);
  repository.findOpenPullRequestsByHead = async (branch) => {
    branches.push(branch);
    return findOpenPullRequestsByHead(branch);
  };

  const result = await handlers.handleCi(event(failed, { relations: [] }));

  assert.deepEqual(result, { kind: 'retry', retry: 1 });
  assert.deepEqual(branches, ['cursor/issue-20-task']);
});
...
test('incoming CI runはofficial path/event/same-repository cursor headとoptional relationが一致しない限りPR APIを読まない', async () => {
  for (const { name, mutate } of [
    ...
    { name: 'non-cursor head branch', mutate: (payload) => { payload.workflow_run.head_branch = 'human/fix'; } },
...
test('複数・malformed・cross-repository・stale relationはmutationなしでfail closedする', async () => {
  for (const { name, relations, mutateRelation } of [
    ...
    { name: 'stale head ref', relations: [relation()], mutateRelation: (reference) => { reference.head.ref = 'cursor/stale'; }, reason: 'invalid-pull-request' },
    { name: 'stale head SHA', relations: [relation()], mutateRelation: (reference) => { reference.head.sha = 'deadbeef'; }, reason: 'invalid-pull-request' },
  ]) {
    const { repository, project, handlers } = setup();
    const failed = run(11, 'failure'); repository.runs.push(failed);
    ...
    assert.deepEqual(result, { kind: 'skip', reason: 'invalid-pull-request' }, name);
    assert.equal(project.status, 'In Progress', name);
    assert.equal(repository.comments.length, 1, name);
  }
});

test('head branchのtrusted REST再解決が0件・複数またはrelation番号不一致ならfail closedする', async () => {
...
test('CI repository gatewayはbranch searchが1000件に達したら不完全履歴としてrejectする', async () => {
  const calls = [];
  const repository = new AgentRepository({
    config: { owner: 'octo-org', repository: 'widgets' },
    client: {
...
test('listCiRunsはempty/dynamic relationを許容しhistorical SHAをtop-level head_shaからmapする', async () => {
  const historicalSha = '619f8c0000000000000000000000000000000000';
  const dynamicCurrentSha = 'ba37c00000000000000000000000000000000000';
  const dynamicRelation = relation();
  dynamicRelation.head.sha = dynamicCurrentSha;
  const otherNumber = relation(31);
  const workflowRun = (id, headSha, pullRequests) => ({
    ...
    client: {
      async read() {
        return { total_count: 3, workflow_runs: [
          workflowRun(1, historicalSha, [dynamicRelation]),
          workflowRun(2, 'abcdef', []),
          workflowRun(3, 'otherhead', [otherNumber]),
        ] };
      },
      async write() {}, async graphql() {},
    },
  ...
  assert.deepEqual(await repository.listCiRuns(currentPullRequest(), 'Pull Request CI'), [
    {
      id: 1, name: 'Pull Request CI', status: 'completed', conclusion: 'success', headSha: historicalSha,
      updatedAt: Date.parse('2026-08-24T00:01:00Z'), url: 'https://ci/1', pullRequests: [{ number: 30 }],
    },
    {
      id: 2, name: 'Pull Request CI', status: 'completed', conclusion: 'success', headSha: 'abcdef',
      updatedAt: Date.parse('2026-08-24T00:02:00Z'), url: 'https://ci/2', pullRequests: [],
    },
  ]);
});

test('listCiRunsは同じbranch名のpublic fork runを無視してsame-repository履歴を返す', async () => {
  const workflowRun = (id, headRepository) => ({
    id, name: 'Pull Request CI', status: 'completed', conclusion: 'success', ...apiRunTrust(),
    head_repository: headRepository,
    updated_at: `2026-08-24T00:0${id}:00Z`, html_url: `https://ci/${id}`,
    repository: { id: 100, full_name: 'octo-org/widgets' }, pull_requests: [],
  });
  const repository = new AgentRepository({
    config: { owner: 'octo-org', repository: 'widgets' },
    client: { async read() { return { total_count: 3, workflow_runs: [
      workflowRun(1, { id: 999, full_name: 'fork/widgets' }),
      workflowRun(2, null),
      workflowRun(3, { id: 100, full_name: 'octo-org/widgets' }),
    ] }; }, async write() {}, async graphql() {} },
  });

  assert.deepEqual(await repository.listCiRuns(currentPullRequest(), 'Pull Request CI'), [{
    id: 3, name: 'Pull Request CI', status: 'completed', conclusion: 'success', headSha: 'abcdef',
    updatedAt: Date.parse('2026-08-24T00:03:00Z'), url: 'https://ci/3', pullRequests: [],
  }]);
});
