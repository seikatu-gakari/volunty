...
Require the incoming `workflow_run` to match name `Pull Request CI`, path `.github/workflows/ci.yml`, event `pull_request_target`, configured repository/head repository and base `main` metadata. Require exactly one official relation whose base matches that top-level metadata and whose head is a same-repository `cursor/*` ref. Resolve its PR number through trusted REST data, then verify the PR is open, Agent-managed and current base/head repository/ref/SHA exactly matches the relation before re-reading the closing Issue/dispatch marker. Query the fixed `/actions/workflows/ci.yml/runs` endpoint by `pull_request_target` and `created>=PR.created_at`, without a branch filter. Runtime-validate every returned run, ignore empty/unrelated/fork relations, and accept only an exact number/base/head ref plus head-repository relation; map the PR head SHA from that relation while treating top-level run head fields as base metadata. Fail closed at the 1,000-result created-window cap. Select the newest current-head target run by `(updated_at,id)` before checking status, so a newer queued/in-progress run blocks both Human Review and stale failure repair. Count trusted operator/run/SHA/range retry markers after the latest successful CI run. Post a `yuto90` comment with run URL and unique marker for retries 1-3; transition to `Blocked` on the next failure. On success evaluate Human Review gate.
...
`agent-review.yml` uses only `workflow_dispatch` with a positive integer PR input, operator-only `github.actor`/`github.triggering_actor` guards, Environment `agent-orchestrator`, and trusted default-branch checkout. `agent-ci.yml` consumes only `Pull Request CI`; before Environment/PAT materialization its job guard fixes event `pull_request_target`, path `.github/workflows/ci.yml`, same-repository run, success/failure conclusion, top-level base `main` metadata and exactly one same-repository `cursor/*` PR relation. It shares the repository-wide lock and never consumes artifacts.
