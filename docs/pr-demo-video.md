...
7. invalidator・publisher・finalizer job自体はActionsの単一pending concurrencyへ入れず、run所有者と取得時刻を記録した専用Git refを原子的に取得して待機する。完了済みrunが残したstale lockは回収し、pending更新・publish・finalize・cleanupのすべてを同じlockで取りこぼしなく直列化して必ず解放する。新CIのinvalidatorはlock待機前にpendingを即時設定し、lock内でも最新runを再同期するため、120分のpublisherがlockを保持していても旧successをReady判定へ残さない。lock取得中のGitHub API一時障害は指数backoffで最大6回試行する。lock待機上限は75分、invalidator・finalizer jobは90分、publish・cleanup jobはlock待機後の処理余裕を含む120分とする。invalidatorはpending更新直後にも最新CIを再照合し、その間に完了した場合はlock解放後のcompleted publisherへ最終状態の確定を引き継ぐ。`gh-pages` はPRごとの最新HEADだけを持つroot commitへ置き換え、公開対象だけを抽出・再検証したtreeをGitHub Pagesへdeployする。
