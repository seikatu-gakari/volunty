...
7. publisher job自体はActionsの単一pending concurrencyへ入れず、run所有者と取得時刻を記録した専用Git refを原子的に取得して待機する。完了済みrunが残したstale lockは回収し、publish・cleanupの両方で `gh-pages` checkoutからPages確認までを直列化して必ず解放する。`gh-pages` はPRごとの最新HEADだけを持つroot commitへ置き換え、公開対象だけを抽出・再検証したtreeをGitHub Pagesへdeployする。
8. SHA固有manifestの反映をHTTP確認し、今回配置したmanifest本文のSHA-256、`gh-pages` の永続化、対象run ID・`run_attempt` が同一HEADの最新CIであることを再照会する。Pages処理とcomment / status更新を別jobに分け、finalizerだけをHEAD単位で直列化する。invalidatorはその単一pending枠から分離して必ず実行し、finalizerもstatus更新後に最新run・attemptを再照合して、途中で新CIが始まっていればpendingへ復元する。job間handoffを取得できない場合もtrusted eventとGitHub APIから最新HEADを再解決してfailureを確定する。すべて一致した場合だけPR commentをupsertし、commit status `demo-video` を成功にする。
