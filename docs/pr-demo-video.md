...
2. 必須checkの `quality` は開始直後、`demo-video` の最新statusが自身のrun ID・`run_attempt` を指すpendingになったことをread-only APIで確認するまで待機する。status lockが競合しても新しいqualityは成功できないため、最新quality / e2eと旧runのdemo successが同時にReady条件を満たさない。105分以内に確認できなければqualityをfail closedにする。
3. `demo-policy` がPR本文、label、変更pathを検証する。
4. 通常の全E2Eを録画なしで実行する。
5. DBを再初期化し、指定シナリオだけをdesktop 1280×720 / mobile 390×844で再実行する。
6. WebMをH.264 MP4と軽量GIFへ変換し、SHA-256・size・録画時間をmanifestへ記録する。decision、Playwright結果、demo結果のartifact名には `run_attempt` を含める。demo-policyはdecision artifact名をjob出力でe2eへ渡すため、e2eだけを再実行しても成功済みproducer attemptのdecisionを取得できる。
7. completed `workflow_run` がmain上のtrusted codeだけをcheckoutし、GitHub APIのliveなPR本文・label・変更pathでpolicyを再評価してartifact decisionと照合する。同一HEADに複数の `Pull Request CI` runまたは再実行attemptがある場合は、対象PRの最新run numberに対応するrun ID・`run_attempt` だけを公開処理へ進め、最新runまたはcurrent HEADの照会に失敗した場合はfailure resultだけを作り、既存の公開treeを変更しない。artifactはcurrent attempt以下で番号が最大の `pr-demo-results-N` を選び、qualityだけのfailed-job再実行などでE2Eが省略された場合は成功済みE2E attemptを引き継ぐ。未来attempt、不正名、同一attemptの重複は拒否する。download前にAPI上の個数・圧縮sizeを制限し、許可file名・展開後size・圧縮率・symlink・path traversalを検証する専用展開処理を通す。さらにMP4のH.264・無音・viewport・時間、GIFのviewport・時間と全frame decode、identity、件数、hashを検証し、PR codeはcheckoutしない。
8. invalidator・publisher・finalizer job自体はActionsの単一pending concurrencyへ入れず、run所有者と取得時刻を記録した専用Git refを原子的に取得して待機する。Pages treeを変更するpublisher・cleanupはPages lock、commit statusを変更するinvalidator・finalizerは独立したstatus lockで直列化する。これにより長時間のPages公開中でも新CIのpending更新を待たせず、lock外の遅延更新で最新successを上書きしない。完了済みrunが残したstale lockは回収し、各lockを必ず解放する。lock取得中のGitHub API一時障害は指数backoffで最大6回試行する。lock待機上限は100分、status lock内のtrusted commandは15分上限として終了時に成否を問わず解放し、invalidator・finalizer・publisher・cleanup jobは120分上限とする。invalidatorはpending更新直後にも最新CIを再照合し、その間に完了した場合はstatus lock解放後のcompleted workflowのfinalizerへ最終状態の確定を引き継ぐ。`gh-pages` はPRごとの最新HEADだけを持つroot commitへ置き換え、公開対象だけを抽出・再検証したtreeをGitHub Pagesへdeployする。
9. 検証済みPages treeを先に履歴なし `gh-pages` へ永続化してからdeployし、SHA固有manifestの反映をHTTP確認する。今回配置したmanifest本文のSHA-256、`gh-pages` の永続化、対象run ID・`run_attempt` が同一HEADの最新CIであることを再照会する。Pages処理とcomment / status更新を別jobに分け、finalizerもdurable lock内で直列化する。通常・手動fallbackの初回freshness照会は指数backoffで最大6回試行し、回復しない場合は既存のcommentとstatusを変更せずfinalizer jobを失敗させる。finalizerはstatus更新後にも最新run・attemptを再照合して、途中で新CIが始まっていればpendingへ復元する。最終再照合自体に失敗した場合は直前のsuccessをfailureへ戻してfail closedにする。publish jobはhandoff artifact名をjob出力でfinalizerへ渡すため、finalizeだけを再実行しても元attemptの結果を取得できる。job間handoffを取得できない場合もtrusted eventとGitHub APIから最新HEADを再解決し、最新性を確認できた場合だけfailureを確定する。すべて一致した場合だけPR commentをupsertし、commit status `demo-video` を成功にする。
