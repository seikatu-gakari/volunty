...
7. invalidator・publisher・finalizer job自体はActionsの単一pending concurrencyへ入れず、run所有者と取得時刻を記録した専用Git refを原子的に取得して待機する。Pages treeを変更するpublisher・cleanupはPages lock、commit statusを変更するinvalidator・finalizerは独立したstatus lockで直列化する。これにより長時間のPages公開中でも新CIのpending更新を待たせず、lock外の遅延更新で最新successを上書きしない。完了済みrunが残したstale lockは回収し、各lockを必ず解放する。lock取得中のGitHub API一時障害は指数backoffで最大6回試行する。lock待機上限は75分、invalidator・finalizer jobは90分、publish・cleanup jobはlock待機後の処理余裕を含む120分とする。invalidatorはpending更新直後にも最新CIを再照合し、その間に完了した場合はstatus lock解放後のcompleted workflowのfinalizerへ最終状態の確定を引き継ぐ。`gh-pages` はPRごとの最新HEADだけを持つroot commitへ置き換え、公開対象だけを抽出・再検証したtreeをGitHub Pagesへdeployする。
8. 検証済みPages treeを先に履歴なし `gh-pages` へ永続化してからdeployし、SHA固有manifestの反映をHTTP確認する。今回配置したmanifest本文のSHA-256、`gh-pages` の永続化、対象run ID・`run_attempt` が同一HEADの最新CIであることを再照会する。Pages処理とcomment / status更新を別jobに分け、finalizerもdurable lock内で直列化する。finalizerの初回freshness照会は指数backoffで最大6回試行し、回復しない場合は対象HEADのcommentとstatusをfailureへ確定する。finalizerはstatus更新後にも最新run・attemptを再照合して、途中で新CIが始まっていればpendingへ復元する。最終再照合自体に失敗した場合は直前のsuccessをfailureへ戻してfail closedにする。publish jobはhandoff artifact名をjob出力でfinalizerへ渡すため、finalizeだけを再実行しても元attemptの結果を取得できる。job間handoffを取得できない場合もtrusted eventとGitHub APIから最新HEADを再解決してfailureを確定する。すべて一致した場合だけPR commentをupsertし、commit status `demo-video` を成功にする。
