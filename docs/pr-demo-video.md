...
7. invalidator・publisher・finalizer job自体はActionsの単一pending concurrencyへ入れず、run所有者と取得時刻を記録した専用Git refを原子的に取得して待機する。完了済みrunが残したstale lockは回収し、pending更新・publish・finalize・cleanupのすべてを同じlockで取りこぼしなく直列化して必ず解放する。lock取得中のGitHub API一時障害は指数backoffで最大6回試行する。lock待機上限は75分、invalidator・finalizer jobは90分、publish jobは公開処理の余裕を含む120分とし、長時間のpublishやcleanup中に始まったCIでも旧successを残さない。invalidatorはpending更新直後にも最新CIを再照合し、その間に完了した場合はlock解放後のcompleted publisherへ最終状態の確定を引き継ぐ。`gh-pages` はPRごとの最新HEADだけを持つroot commitへ置き換え、公開対象だけを抽出・再検証したtreeをGitHub Pagesへdeployする。
8. SHA固有manifestの反映をHTTP確認し、今回配置したmanifest本文のSHA-256、`gh-pages` の永続化、対象run ID・`run_attempt` が同一HEADの最新CIであることを再照会する。Pages処理とcomment / status更新を別jobに分け、finalizerもdurable lock内で直列化する。finalizerはstatus更新後に最新run・attemptを再照合して、途中で新CIが始まっていればpendingへ復元する。最終再照合自体に失敗した場合は直前のsuccessをfailureへ戻してfail closedにする。publish jobはhandoff artifact名をjob出力でfinalizerへ渡すため、finalizeだけを再実行しても元attemptの結果を取得できる。job間handoffを取得できない場合もtrusted eventとGitHub APIから最新HEADを再解決してfailureを確定する。すべて一致した場合だけPR commentをupsertし、commit status `demo-video` を成功にする。
