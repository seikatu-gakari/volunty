...
8. SHA固有manifestの反映をHTTP確認し、今回配置したmanifest本文のSHA-256、`gh-pages` の永続化、対象run ID・`run_attempt` が同一HEADの最新CIであることを再照会する。Pages処理とcomment / status更新を別jobに分け、最終更新jobとinvalidatorをHEAD単位の同じ排他lockでcancelせず直列化する。invalidatorは先行finalizerの終了後に最新run・attemptを再照合するため、古いrunの再実行eventが最新finalizerをcancelしない。job間handoffを取得できない場合もtrusted eventとGitHub APIから最新HEADを再解決してfailureを確定する。すべて一致した場合だけPR commentをupsertし、commit status `demo-video` を成功にする。
