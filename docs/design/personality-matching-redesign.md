...
- **簡易版（15項目）を提供する**（2026-07-06 追記。方針変更の経緯は下記参照）。
- **ファセットスコアは提供しない**。ファセットを提供する条件は「ファセット構造が日本語データで確認できていること」であり、現時点で満たさない。

### 3.1 簡易版（15項目）追加の経緯（2026-07-06 追記）

初版（2026-07-04）では上記の理由により簡易版を提供しない方針としたが、
「スキマ時間でざっくり傾向を知りたい」というプロダクト要望を受け、
以下の設計で簡易版を追加した。初版で指摘した懸念は解消していないため、
懸念を回避する設計上の工夫とセットで導入する。

- **初版の懸念は残る**: 特性あたり3項目という項目数は、初版が問題視した
  「特性あたり3〜4問では内的一貫性を担保できない」という懸念をそのまま抱える。
  簡易版はあくまで「傾向をすばやく把握する」目的に限定し、精緻な測定の代替とはしない。
- **2モード混在によるスコア精度の不透明化を回避する設計**: 初版が問題視したのは
  「簡易/詳細の2モードが同一尺度上のスコアとして混在保存され、精度の異なるスコアが
  区別なくマッチングに使われる」ことだった。これを避けるため、簡易版は独立した
  `scaleCode: ipip-bfm-50-ja-brief15` として管理し、50項目版とスコアを混同しない
  （DBの `scaleCode` 列で常に判別可能。マッチングエンジン・団体向け表示のロジックは
  scaleCode を意識せず scaled score のみを参照するため変更不要）。
- **項目の選定**: IPIP-BFM-50-JA の一次資料の出題順（displayOrder）で、
  各ドメインが5項目ずつ循環する構造を利用し、`displayOrder 1〜15`
  （＝各ドメイン先頭3項目）をそのまま抜粋した。項目文言の改変・独自項目の追加は行わない。
- **未検証であることの明示**: Apple & Neff (2012) の検証は50項目全体に対するものであり、
  この15項目抜粋自体の内的一貫性・妥当性は検証されていない
  （`ScaleDefinition.excerptCaveat` に明記し、結果画面でも簡易版であることと
  全50問への案内を表示する）。
- **回答品質判定の項目数依存の解消**: `too_fast` は元々項目数に比例した閾値だったため
  無変更で機能する。`straight_lining` は旧実装が「50項目中15問以上」という絶対値だったため、
  「全項目数の30%（最低5問）」という相対閾値に一般化した（50項目なら従来と同じ15問）。
...
t_diagnosis_response（新設: 生回答）
  ...
  ※ 保存目的: 品質検証・尺度の自社検証（§17）。常に保存する
    （2026-07-06 追記: 初版では同意チェックボックス任意・デフォルトOFFだったが、
    プロダクト判断により同意確認UIを撤廃し常時保存に変更した）
...
- 団体へ生スコアを開示しない（§8.4）
- 生回答は常に保存し、目的（品質検証・尺度検証）を明示。アカウント削除で連鎖削除。24ヶ月で仮名化
  （2026-07-06 追記: 同意チェックボックスは撤廃し、常時保存に変更）
