...
`pull_request_target`から生じる`workflow_run`ではtop-levelの`head_branch` / `head_sha`はPR headではなくbase `main`のmetadataである。`agent-ci.yml`はEnvironment/PATをmaterializeする前に、exactly oneの`pull_requests` relationがsame-repository `cursor/*` PRを指し、そのbaseがtop-level metadataと一致することをguardする。handlerもrelationのPR番号をRESTで再取得し、open/managed/base/head repository・ref・current SHAを照合する。CI履歴はPR作成日時以後のfixed workflow/eventを検索し、各runのexact relationからPR head SHAを対応付けるため、top-level値をPR headとして使わない。
